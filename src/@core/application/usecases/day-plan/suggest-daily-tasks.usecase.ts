...
export class SuggestDailyTasksUseCase
  implements IUseCase<ISuggestDailyTasksInput, ISuggestDailyTasksOutput>
{
  constructor(
  ...
  async execute(
    input: ISuggestDailyTasksInput
  ): Promise<ISuggestDailyTasksOutput> {
    const {
      targetDate,
      workStartTime,
      workEndTime,
    ...
    const { workRoutines, nonWorkRoutines } =
      this.categorizeRoutines(activeRoutines);
    ...
    const aiPrompt = this.generateIntelligentAIPrompt(
      scheduleConfig,
      date,
      userContext
    );
    ...
    try {
      ...
      const fallbackPrefs = {
        workStartHour: scheduleConfig.workStartTime
          ? parseInt(scheduleConfig.workStartTime.split(":")[0])
          : 9,
        workEndHour: scheduleConfig.workEndTime
          ? parseInt(scheduleConfig.workEndTime.split(":")[0])
          : 17,
        includeBreaks: true,
        breakDuration: 15,
        maxTasksPerHour: 2,
      ...
      return this.generateFallbackPlan(
        [...scheduleConfig.workTasks, ...scheduleConfig.personalTasks],
        [...scheduleConfig.workRoutines, ...scheduleConfig.nonWorkRoutines],
        date,
        fallbackPrefs
      );
    }
  ...
  private categorizeTasks(tasks: Task[]): {
    workTasks: Task[];
    personalTasks: Task[];
  } {
  ...
  private categorizeRoutines(routines: Routine[]): {
    workRoutines: Routine[];
    nonWorkRoutines: Routine[];
  } {
...
- Alta prioridade: ${stats.high} itens (${
      total > 0 ? Math.round((stats.high / total) * 100) : 0
    }%)
- Média prioridade: ${stats.medium} itens (${
      total > 0 ? Math.round((stats.medium / total) * 100) : 0
    }%)
- Baixa prioridade: ${stats.low} itens (${
      total > 0 ? Math.round((stats.low / total) * 100) : 0
    }%)
- OBJETIVO: Inclua pelo menos 70% das HIGH, 50% das MEDIUM e 30% das LOW`;
    ...
    switch (routine.frequency) {
      ...
      case "monthly":
        ...
        if (routine.daysOfMonth)
          return routine.daysOfMonth.includes(dayOfMonth);
        return false;
      case "yearly":
  ...
  private buildUserContext(prompts: UserPrompt[]): string {
    if (prompts.length === 0)
      return "Usuário sem contexto específico definido.";
  ...
  private generateIntelligentAIPrompt(
    ...
    if (hasWorkRoutine) {
      ...
      workTasks.forEach((task: Task, idx: number) => {
        prompt += `\n${idx + 1}. [${task.priority.toUpperCase()}] ${
          task.title
        }\n   Descrição: ${
          task.description
      ...
      if (workRoutines.length > 0) {
        ...
        workRoutines.forEach((routine: Routine, idx: number) => {
          prompt += `\n${idx + 1}. [${routine.priority.toUpperCase()}] ${
            routine.title
      ...
      prompt += `\n\nREGRAS PARA DIA SEM TRABALHO:\n1. Atividades podem ser distribuídas livremente entre ${wakeUpTime} e ${sleepTime}\n2. Priorize tarefas de alta importância nos horários de maior energia\n3. Distribua atividades de forma equilibrada\n\nTODAS AS TAREFAS (${
        workTasks.concat(personalTasks).length
      }):`;
      ...
      if ([...workRoutines, ...nonWorkRoutines].length > 0) {
        prompt += `\n\nROTINAS ATIVAS (${
          [...workRoutines, ...nonWorkRoutines].length
        }):`;
    ...
    const priorityStats = this.generatePriorityStats(
      ...
      [...workRoutines, ...nonWorkRoutines]
    );

    prompt += `\n\n${priorityStats}\n\nCONFIGURAÇÕES ADICIONAIS:\n- Áreas de foco: ${
      focusAreas.join(", ") || "Nenhuma específica"
...
Responda APENAS com um JSON válido seguindo esta estrutura:
{
  ...
  private formatOutput(
    ...
    const totalActivities =
      aiResponse.hourlyPlan?.reduce(
        (acc: number, slot: HourlyPlanSlot) =>
          acc + (slot.activities?.length || 0),
        0
      ) || 0;
    return {
  ...
  private generateFallbackPlan(
    ...
    let routineIndex = 0;
    while (
      currentHour < prefs.workEndHour &&
      (taskIndex < tasks.length || routineIndex < routines.length)
    ) {
    ...
    return {
      ...
      motivationalMessage:
        "Cada pequeno passo te leva mais perto dos seus objetivos. Você consegue!",
    };
  }
}
